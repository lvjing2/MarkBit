...
package com.liwn.zzl.markbit.listener;
...
public class DrawingSurfaceListener implements OnTouchListener {
	...
	public DrawingSurfaceListener() {
		mPerspective = MarkBitApplication.perspective;
	...
	public boolean onTouch(View view, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			MarkBitApplication.currentTool.handleDown(touchPoint);
		...
		case MotionEvent.ACTION_MOVE:
			if (event.getPointerCount() == 1) {
				...
				MarkBitApplication.currentTool.handleMove(touchPoint);
		...
		case MotionEvent.ACTION_CANCEL:
			...
			if (mTouchMode == TouchMode.DRAW) {
				MarkBitApplication.currentTool.handleUp(touchPoint);
			} else {
	...
	private class MoveThread extends Thread {
		...
		protected MoveThread() {
			...
			running = !ignoredTools.contains(MarkBitApplication.currentTool
					.getToolType());
			scrolling = false;
		}
		...
		public void run() {
			while (running) {
				Point autoScrollDirection = MarkBitApplication.currentTool
						.getAutoScrollDirection(pointX, pointY, width, height);

				if (autoScrollDirection.x != 0 || autoScrollDirection.y != 0) {
					scrolling = true;

					MarkBitApplication.perspective.translate(
							autoScrollDirection.x * step, autoScrollDirection.y
									* step);
					PointF newMovePoint = MarkBitApplication.perspective
							.getCanvasPointFromSurfacePoint(new PointF(pointX,
									pointY));
					MarkBitApplication.currentTool.handleMove(newMovePoint);
				}

				try {
					sleep(calculateScrollInterval(MarkBitApplication.perspective
							.getScale()));
