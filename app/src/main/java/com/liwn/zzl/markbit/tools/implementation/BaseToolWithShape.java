...
import com.liwn.zzl.markbit.tools.ToolWithShape;

public abstract class BaseToolWithShape extends BaseTool implements
		ToolWithShape {

	protected int mPrimaryShapeColor = MarkBitApplication.applicationContext
			.getResources().getColor(R.color.rectangle_primary_color);
	protected int mSecondaryShapeColor = MarkBitApplication.applicationContext
	...
	public BaseToolWithShape(Context context, ToolType toolType) {
		...
		float actionBarHeight = DrawOptionsMenuActivity.ACTION_BAR_HEIGHT
				* metrics.density;
	...
	protected float getInverselyProportionalSizeForZoom(float defaultSize) {
		float displayScale = mContext.getResources().getDisplayMetrics().density;
		float applicationScale = MarkBitApplication.perspective.getScale();
	...
	public Point getAutoScrollDirection(float pointX, float pointY,
		...
		int deltaX = 0;
		int deltaY = 0;
		PointF surfaceToolPosition = MarkBitApplication.perspective
