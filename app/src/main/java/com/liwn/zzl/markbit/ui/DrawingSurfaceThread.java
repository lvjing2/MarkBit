...
class DrawingSurfaceThread {
	...
	synchronized void start() {
		Log.d(MarkBitApplication.TAG, "DrawingSurfaceThread.start");
		...
		// Log.d(MarkBitApplication.TAG, "DrawingSurfaceThread.start up");
		if (!internalThread.isAlive()) {
			running = true;
			internalThread.start();
			// Log.d(MarkBitApplication.TAG, "DrawingSurfaceThread.started");
		}
	}

	synchronized void stop() {
		Log.d(MarkBitApplication.TAG, "DrawingSurfaceThread.stop");
		running = false;
		if (internalThread != null && internalThread.isAlive()) {
			Log.w(MarkBitApplication.TAG, "DrawingSurfaceThread.join");
			boolean retry = true;
			while (retry) {
				try {
					internalThread.join();
	...
	synchronized void setRunnable(Runnable runnable) {
		Log.d(MarkBitApplication.TAG, "DrawingSurfaceThread.setRunnable");
		threadRunnable = runnable;
	}
}
